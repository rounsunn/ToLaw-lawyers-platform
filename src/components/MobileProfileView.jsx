...
const MobileProfileView = ({ lawyerData, handleEditButton, handleLogout }) => {
  ...
  return (
    <div className="w-screen h-screen flex flex-col font-poppins">
      ...
      <div
        className="absolute top-24 h-50 w-full bg-cover bg-center z-1 p-2"
      ...
      <div className="flex-grow flex flex-col justify-end">
        ...
        <div className="bg-[#F6FAFC] items-end z-20 p-4 border-black border-1 rounded-t-[27px]">
          ...
          <div className="flex justify-between items-center my-2">
            <div className="flex flex-column items-start gap-2">
              ...
              {lawyerData ? (
                <h2 className="text-sm italic">
                  Bar Council Number - {lawyerData?.barCouncilNumber}
                </h2>
              ) : (
            ...
            {lawyerData ? (
              <img
                ...
                alt="profile-pic"
                className="w-24 h-24 object-cover rounded-full border-black border-[1px]"
              />
            ) : (
          ...
          <div className="my-3">
            <h2 className="text-neutral-800 text-md font-normal mb-1">
              Area of Practice
            </h2>
            {lawyerData ? (
              <div className="flex gap-2 flex-wrap">
                {lawyerData?.lawArea.map((item, index) => (
                  <p
                    key={index}
                    className="opacity-70 text-slate-900 font-normal text-sm p-1 rounded-md border-1 border-blue-900 capitalize"
                  >
                    {item}
                  </p>
                ))}
          ...
          <div className="my-3">
            <div>
              <h3 className="text-neutral-800 text-md font-normal">
                Biography
              </h3>
              {lawyerData ? (
                <p className="text-sm text-gray-400 tracking-wide">
                  {lawyerData?.biography}
                </p>
              ) : (
          ...
          <div className="flex flex-column gap-[2px] my-3">
            {lawyerData ? (
              <div className="flex gap-2 items-center mb-1">
                ...
                <p className="text-sm">
                  {lawyerData?.experience}+ years of Experience
                </p>
              </div>
            ) : (
          ...
          <div className="flex flex-col gap-1 my-3">
            <h2 className="text-neutral-800 text-md font-normal">
              Consultation Services
            </h2>
            <div className="flex justify-between items-center gap-5">
              {lawyerData ? (
                <div className="text-neutral-800 text-sm font-normal">
                  Duration - {lawyerData?.consultingDuration} minutes
                </div>
              ) : (
          ...
          <div className="flex gap-2 mt-2">
            <button
              ...
              className="w-full bg-[#0C253F] text-white py-2 rounded-md"
            >
              Edit your Profile
            </button>
            <button
