    ...
    useEffect(() => {
      ...
      useEffect(() => {
        setFormData({
            ...
            lawCertificate: null,
            charges: lawyerData?.charges || 0,
            consultingDuration: lawyerData?.consultingDuration || '',
            consultingTime: lawyerData?.consultingTime || '',
            qualification: lawyerData?.qualification || '',
            biography: lawyerData?.biography || '',
            emailId: lawyerData?.emailId || '',
            mobileNumber: lawyerData?.mobileNumber || '',
      ...
      async function patch(url, formData) {
        try {
      ...
      async function upload(file) {
        try {
          ...
          if (file) { // Check if file is not null
            ...
            return null; // Return null or handle the case when file is null
          }
        } catch (error) {
          console.error("Error uploading file:", error);
          // Handle the error as needed
          return null; // Return null or handle the case when there's an error
        }
      }
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          if (formData.profilePic instanceof File) {
            // A new profile picture file is selected, upload it
            const profilePicImageUrl = await upload(formData.profilePic);
            newLawyer.profilePic = profilePicImageUrl;
          }
    
          await patch(`/lawyers/${lawyerId}`, newLawyer);
  ...
  return (
    <div className="container my-5">
      <div className='w-full flex justify-between items-center mb-3'>
        <h1 className='text-3xl font-semibold'>Edit Your Details</h1>
        <button className='bg-blue-500 text-white px-2 py-1 rounded-md' onClick={handleLogout}>
          Log out
        </button>
      </div>
